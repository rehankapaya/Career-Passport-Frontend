    ...
    try {
      ...
      const enriched = await Promise.all(
        data.map(async (bookmark) => {
          try {
            // Step 1: Identify localStorage key
            let cacheKey = "";
            if (bookmark.itemType === "resource")
              cacheKey = "resources_cache_v1";
            else if (bookmark.itemType === "multimedia")
              cacheKey = "multimedia_cache_v1";
            else if (bookmark.itemType === "career")
              cacheKey = "career_bank_cache_v1";
            else if (bookmark.itemType === "story")
              cacheKey = "success_stories_cache_v1";

            let displayTitle = "Bookmarked Item";
            let itemData = null;

            // Step 2: Try to fetch from localStorage
            if (cacheKey) {
              ...
              if (cacheRaw) {
                ...
                itemData = cache.items.find(
                  (i) =>
                    i._id === bookmark.itemId ||
                    i.career_id === bookmark.itemId ||
                    i.resource_id === bookmark.itemId ||
                    i.media_id === bookmark.itemId ||
                    i.story_id === bookmark.itemId
                );
              }
            }

            // Step 3: If not found locally, fallback to API
            if (!itemData) {
              let endpoint = "";
              if (bookmark.itemType === "resource")
            ...
            // Step 4: Assign a display title
            if (itemData) {
              displayTitle =
                bookmark.itemType === "story"
                  ? itemData.rname || itemData.title || displayTitle
                  : itemData.title || displayTitle;
            }

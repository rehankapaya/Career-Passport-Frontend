...
export default function CareerBankPage() {
  ...
  const domains = ['All Industries', ...new Set(careerPaths.map(career => career.domain))];

  // Function to get appropriate icon based on career title
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', fontFamily: 'sans-serif', padding: '2rem' }}>
      <div style={{ maxWidth: '80rem', margin: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ color: '#4b5563' }}>Showing {filteredCareers.length} of {careerPaths.length} careers</div>
        </div>

        {/* Career Cards Grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem', marginBottom: '2.5rem' }}>
          {filteredCareers.map((career) => (
            <div key={career._id} style={{ backgroundColor: '#ffffff', borderRadius: '0.5rem', padding: '1.5rem', boxShadow: '0 1px 2px 0 rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                <div style={{ backgroundColor: '#e5e7eb', borderRadius: '0.5rem', padding: '0.5rem', display: 'inline-block', marginBottom: '1rem' }}>
                  <span style={{ fontSize: '1.5rem' }}>{getCareerIcon(career.title)}</span>
                </div>
                ...
                <div style={{ fontSize: '0.875rem', marginBottom: '1rem' }}>
                  <span style={{ fontWeight: '500' }}>Education Path:</span> <br/>{career.education_path}
                </div>
              </div>
              <div>
